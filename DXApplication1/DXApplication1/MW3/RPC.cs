...
namespace DXApplication1
{
	public static class RPC
	{
		...
		private static void WriteSingle(uint address, float input)
		{
			...
			Connection.PS3.SetMemory(address, array);
		}
		...
		{
			Connection.PS3.GetMemory(Address, Bytes);
		}
		...
		public static void Enable()
		{
			...
			Connection.PS3.SetMemory(RPC.func_address, new byte[]
			{
				65
			});
			Connection.PS3.SetMemory(RPC.func_address + 4u, buffer);
			Connection.PS3.SetMemory(RPC.func_address, new byte[]
			{
				64
			});
		...
		public static int Call(uint address, params object[] parameters)
		{
			...
			while (i < num)
			{
				...
				{
					Connection.PS3.Extension.WriteInt32(268763136u + num2 * 4u, (int)parameters[i]);
					num2 += 1u;
				}
				else
				{
					...
					{
						Connection.PS3.Extension.WriteUInt32(268763136u + num2 * 4u, (uint)parameters[i]);
						num2 += 1u;
					}
					else
					{
						...
						{
							Connection.PS3.Extension.WriteInt16(268763136u + num2 * 4u, (short)parameters[i]);
							num2 += 1u;
						}
						else
						{
							...
							{
								Connection.PS3.Extension.WriteUInt16(268763136u + num2 * 4u, (ushort)parameters[i]);
								num2 += 1u;
							}
							else
							{
								...
								{
									Connection.PS3.Extension.WriteByte(268763136u + num2 * 4u, (byte)parameters[i]);
									num2 += 1u;
								}
								else
								{
									if (parameters[i] is string)
									{
										...
										Connection.PS3.Extension.WriteString(num6, Convert.ToString(parameters[i]));
										Connection.PS3.Extension.WriteUInt32(268763136u + num2 * 4u, num6);
										num2 += 1u;
										num3 += 1u;
									}
									else
			...
			Connection.PS3.Extension.WriteUInt32(268763208u, address);
			Thread.Sleep(20);
			return Connection.PS3.Extension.ReadInt32(268763212u);
		}
		public static void DestroyAll()
		{
			byte[] buffer = new byte[184320];
			Connection.PS3.SetMemory(15786252u, buffer);
		}
		...
		public static byte[] GetMemory(uint offset, int length)
		{
			...
			Connection.PS3.GetMemory(offset, array);
			return array;
		}
        ...
        public static void ExecuteCMD(int int_0, object object_1)
        {
            RPC.Call(1946176u, new object[]
            {
                int_0,
                object_1
            });
